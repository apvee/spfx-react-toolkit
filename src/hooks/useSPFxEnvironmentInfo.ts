...
/**
 * Hook to check if running in Microsoft Teams
 * Shortcut for useSPFxEnvironmentInfo().isTeams
 * 
 * @returns true if in Teams, false otherwise
 * 
 * @example
 * ```tsx
 * function MyComponent() {
 *   const isTeams = useSPFxIsTeams();
 *   
 *   return isTeams ? <TeamsUI /> : <SharePointUI />;
 * }
 * ```
 */
export function useSPFxIsTeams(): boolean {
  const { isTeams } = useSPFxEnvironmentInfo();
  return isTeams;
}

/**
 * Hook to check if running in SharePoint Online
 * Shortcut for useSPFxEnvironmentInfo().isSharePoint
 * 
 * @returns true if in SharePoint Online, false otherwise
 * 
 * @example
 * ```tsx
 * function MyComponent() {
 *   const isSPO = useSPFxIsSharePoint();
 *   
 *   if (!isSPO) {
 *     return <div>SharePoint Online only feature</div>;
 *   }
 *   
 *   return <SharePointOnlineFeature />;
 * }
 * ```
 */
export function useSPFxIsSharePoint(): boolean {
  const { isSharePoint } = useSPFxEnvironmentInfo();
  return isSharePoint;
}

/**
 * Hook to check if running in local workbench
 * Shortcut for useSPFxEnvironmentInfo().isLocal
 * 
 * @returns true if in local workbench, false otherwise
 * 
 * @example
 * ```tsx
 * function MyComponent() {
 *   const isLocal = useSPFxIsLocal();
 *   
 *   return (
 *     <div>
 *       {isLocal && <div style={{ background: 'yellow' }}>DEV MODE</div>}
 *       <Content />
 *     </div>
 *   );
 * }
 * ```
 */
export function useSPFxIsLocal(): boolean {
  const { isLocal } = useSPFxEnvironmentInfo();
  return isLocal;
}

/**
 * Hook to check if running in workbench (local or hosted)
 * Shortcut for useSPFxEnvironmentInfo().isWorkbench
 * 
 * @returns true if in workbench, false otherwise
 * 
 * @example
 * ```tsx
 * function MyComponent() {
 *   const isWorkbench = useSPFxIsWorkbench();
 *   
 *   if (isWorkbench) {
 *     return <div>Testing in workbench...</div>;
 *   }
 *   
 *   return <ProductionUI />;
 * }
 * ```
 */
export function useSPFxIsWorkbench(): boolean {
  const { isWorkbench } = useSPFxEnvironmentInfo();
  return isWorkbench;
}
